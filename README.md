...
This does NOT produce any official file format for the frames but only offers the matrices U,S,V with Frame=U*S*V'. The compression happens since U and V both have only r columns and S is an rxr matrix with r being the chosen compression rank. (See singular value decomposition for the concept). This offers a fast calculation of these decompositions of a series of frames or a time dependend differential equation of order 1.
