function [Frames, r] = DLR(StartA, frameCount, GetDelta, MakeFrame, approx, approxAsRank)
...
%   as the approximation rank to use. Approx should therefore be an int >=1
%OUTPUT:
...
if nargin >= 6 && strcmp(approxAsRank, 'asrank') && approx >= 1
    ...
    % additionally
    r = approx/2*sqrt(prod(frameDim));
end

%fixed approximation rank r >= 1, r <= n,m (dimensions of single frames)
r = floor(min(frameDim(2), min(frameDim(1), max(1, r)))); 
disp(strcat('Starting to calculate DLR for rank=', num2str(r), '; true rank of starting matrix A=', num2str(rank(StartA))));
